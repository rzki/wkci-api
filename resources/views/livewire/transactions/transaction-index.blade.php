                                        ...
                                        <div class="col d-flex justify-content-end pb-3">
                                            <a href="{{ route('transactions.qr_manual') }}"
                                                class="btn btn-success ml-3 text-white" wire:navigate><i
                                                    class="fa fa-plus"
                                                    aria-hidden="true"></i>{{ __(' Create Manual Transaction') }}</a>
                                        </div>
                                    </div>
                                    ...
                                    <div class="row export">
                                        <div class="col">
                                            ...
                                            <button wire:click="deleteSelected"
                                                class="btn btn-danger {{ count($selectedItems) ? '' : 'd-none' }}">{{ __('Delete Selected Data') }}</button>
                                            <button wire:click="bulkUpdatePaymentStatus"
                                                class="btn btn-success text-white {{ count($selectedItems) ? '' : 'd-none' }}">{{ __('Update payment status selected data') }}</button>
                                        </div>
                                    </div>
                                                    ...
                                                    @foreach ($transactions as $trx)
                                                        <tr>
                                                            <td><input type="checkbox" name="selectedItems"
                                                                    id="selectedItems" wire:key="{{ $trx->id }}"
                                                                    value="{{ $trx->id }}"
                                                                    wire:model.live="selectedItems"></td>
                                                            ...
                                                            @elseif($trx->amount == '0,00')
                                                                <td>{{ 'IDR 0,00' }}</td>
                                                            @else
                                                                <td>{{ 'IDR ' . number_format($trx->amount, 2, '.', ',') }}
                                                                </td>
                                                            @endif
                                                            @if ($trx->trx_proof != null)
                                                                <td>
                                                                    <a href="{{ $trx->trx_proof }}" target="_blank"
                                                                        class="text-decoration-underline text-info"><i
                                                                            class="fas
                                                        fa-up-right-from-square"></i>
                                                                        {{ __(' Lihat Bukti Transfer') }}</a>
                                                                </td>
                                                            @else
                                                                <td></td>
                                                            @endif
                                                            <td>{{ date('d/m/Y H:i:s', strtotime($trx->paid_at)) ?? '' }}
                                                            </td>
